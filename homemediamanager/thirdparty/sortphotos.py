...
def sortPhotos(src_dir, dest_dir, sort_format, rename_format, recursive=False,
    ...
    for idx, data in enumerate(metadata):
        ...
        if rename_format is not None:
            orig, ext = os.path.splitext(filename)
            filename = date.strftime(rename_format) + ext
            if '{orig}' in rename_format:
                filename = filename.replace('{orig}', orig)
        ...
        if test:
            ...
            matt_file_results.append({'src_file': src_file, 'dest_file': dest_file})
